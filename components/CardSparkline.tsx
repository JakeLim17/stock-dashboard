...
//
//   - mount 시 /api/sparkline?code=... fetch (서버 30s 캐시 + 클라 60s 폴링).
...
//
// 성능 정책:
//   - 카드 6개 × 60s 폴링 → 분당 6 fetch. 서버 캐시 30s 라 외부 호출은 절반.
//   - 폴링은 "탭 활성"일 때만 — visibility 변화로 자동 시작/정지.
...
interface Props {
  ...
  height?: number;
  /** 폴링 주기. 기본 60s — 분봉 어차피 1분 단위로 의미 갱신. */
  pollMs?: number;
