...
# KIS WebSocket 실시간 가격 (Phase 1 — 체결가만)

> 카드의 현재가가 토스 증권 수준으로 즉시 갱신되도록 KIS WebSocket H0STCNT0(체결가) 를
> SSE 로 클라이언트까지 흘려보내는 모드. **기본 OFF**. 호가/체결 패널은 여전히 REST polling 사용.

## 흐름 1줄

브라우저 ─SSE EventSource→ `/api/realtime/stream?symbols=...` ─KIS WS H0STCNT0→ KIS
(서버가 KIS 메시지를 `event: price\ndata: {code,price,ts}` 로 변환해 push.)
...
3. 브라우저에서 카드를 보면 한국 종목 현재가가 SSE tick 마다 즉시 갱신.

## 비활성화 (기본)

- `NEXT_PUBLIC_REALTIME_ENABLED` 가 `true` 가 아니면 `EventSource` 자체를 안 만든다.
- 모든 카드는 기존 5초 REST polling 으로 동작 — **회귀 0**.
...
- **동시 구독 최대 41건** (KIS 측 한도). 현재 watchlist 가 6개라 여유.
...
## 호가/체결 실시간 (H0STASP0) 은?

- **이번 Phase 1 에 미포함.** 호가 패널은 기존 REST polling + 깜빡임 fix (3회 연속 실패 후에만 빈 메시지) 로 그대로 유지.
- 추후 필요 시 별도 작업으로 추가 가능 (라우트 topics 파라미터 + `useRealtime` 에 `asps` 반환 추가).

## 코드 위치

- `lib/providers/kisApproval.ts` — approval_key 발급/캐시 (REST 토큰과 별도)
- `app/api/realtime/stream/route.ts` — Node.js SSE 엔드포인트 + KIS WS 클라이언트
- `hooks/useRealtime.ts` — 클라이언트 EventSource + throttle(200ms) + auto-reconnect
- `components/StockCard.tsx` — `priceOverride?: number | null` prop
- `components/DashboardClient.tsx` — `useRealtime(visibleCodes)` 호출 + 카드에 전파
