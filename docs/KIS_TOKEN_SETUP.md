...
1. Vercel 환경변수 추가:
   ...
   - `KIS_WS_URL` 는 **선택**. 비워 두면 `KIS_BASE_URL` 이 모의(`openapivts`) 이면 `wss://ops.koreainvestment.com:31000`,
     실전이면 `wss://ops.koreainvestment.com:21000` 으로 자동 분기.
     명시하려면 `wss://...` 풀 URL 만 (KEY=value 통째로 박으면 정규화 함수가 잘라냄).
2. **Redeploy** 한 번 — `NEXT_PUBLIC_*` 는 클라이언트 빌드에 박혀야 active.
   Vercel "Build Cache" 체크는 **해제** 권장 — 옛 bundle 잔존 방지.
3. 브라우저에서:
